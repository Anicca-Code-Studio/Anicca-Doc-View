/**
 * AniccaClient - SDK entry point for document viewing.
 *
...
import { AniccaViewer } from "./AniccaViewer.js";
import type {
    ScrollMode,
    LayoutMode,
    ZoomMode,
...
/**
 * Options for creating a AniccaClient.
 */
 ...
 */
export type DocumentSource = string | File | Uint8Array | AniccaViewer;
...
export class AniccaClient {
    ...
    private viewers: Set<AniccaViewer> = new Set();
     ...
     */
    static async create(options: ClientOptions = {}): Promise<AniccaClient> {
        ...
        const cdnUrl = `https://cdn.jsdelivr.net/npm/anicca-doc-view@${AniccaClient.version}/dist/src/wasm/anicca-engine_bg.wasm`;
        const wasmUrls: string[] = [];

        if (options.baseUrl) {
            wasmUrls.push(new URL("anicca-engine_bg.wasm", options.baseUrl).href);
        } else {
            try {
        ...
        for (const wasmUrl of wasmUrls) {
            try {
                await workerClient.init(wasmUrl, options.__experimentalGpu, domain, AniccaClient.version);
                lastError = null;
                break;
            } catch (e) {
                lastError = e;
        ...
        const client = new AniccaClient(workerClient, options);
        ...
        if (!options.disableUpdateCheck) {
            checkForUpdates(AniccaClient.version);
        }
     ...
     * ```
     */
    async compose(compositions: Pick[][]): Promise<AniccaViewer[]> {
        this.ensureNotDestroyed();
        ...
        for (const [source, docId] of sourceMap) {
            if (!(source instanceof AniccaViewer)) {
                await this.workerClient.unloadPdf(docId);
            }
        }

        // Create viewers for the composed documents
        const viewers: AniccaViewer[] = [];
        for (const docId of newDocIds) {
            const viewer = new AniccaViewer(this.workerClient, {}, true, true, AniccaClient.version);
    ...
    ): Promise<{ viewers: AniccaViewer[]; sections: { title: string; index: number }[] }> {
        this.ensureNotDestroyed();

        const { maxLevel = 1, splitMidPage = false } = options;
        const docId = await this.loadSource(source);
        const isTemporary = !(source instanceof AniccaViewer);
    ...
    private ensureNotDestroyed(): void {
        if (this.destroyed) {
            throw new Error("AniccaClient has been destroyed");
        }
    }
}
...
function checkForUpdates(currentVersion: string): void {
    fetch("https://registry.npmjs.org/anicca-doc-view/latest")
        .then((res) => (res.ok ? res.json() : null))
        .then((data) => {
            if (!data?.version) return;
            if (data.version !== currentVersion) {
                console.warn(
                    `[anicca-doc-view] A newer version is available: ${data.version} (current: ${currentVersion}). ` +
                        `Update with: npm install anicca-doc-view@latest\n` +
                        `To disable this check, set { disableUpdateCheck: true } in AniccaClient.create() options.`,
                );
            }
        })
        .catch(() => {
