...
export class Wasm {
     ...
     * # Example (JavaScript)
     * ```js
     * engine.enableGoogleFonts();
     * const docId = engine.loadPptx(pptxBytes);
     * const pixels = engine.renderPageToRgba(docId, 0, 800, 600);
     * ```
     */
     ...
     * const newDocIds = engine.pdf_compose(
     *   [
     *     [{ doc: 0, pages: "0-2" }],
     ...
     * # Example (JavaScript)
     * ```js
     * const annotations = engine.get_all_annotations(docId);
     * // ... viewer edits annotations ...
     * const pdfBytes = engine.pdf_save_annotations(docId, annotations);
     * ```
     */
