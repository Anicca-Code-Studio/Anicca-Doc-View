...
export function mountViewerShell(
    ...
    layout.className = "adv-viewer-root";
    ...
    toolbarSlot.className = "adv-slot adv-toolbar-slot";

    const subToolbarSlot = document.createElement("div");
    subToolbarSlot.className = "adv-slot adv-subtoolbar-slot";

    const bodySlot = document.createElement("div");
    bodySlot.className = "adv-slot adv-body-slot";

    const leftPanelSlot = document.createElement("div");
    leftPanelSlot.className = "adv-slot adv-left-panel-slot";

    const viewportSlot = document.createElement("div");
    viewportSlot.className = "adv-slot adv-viewport-slot";

    const rightPanelSlot = document.createElement("div");
    rightPanelSlot.className = "adv-slot adv-right-panel-slot";
    ...
    skipLink.href = "#";
    skipLink.className = "adv-skip-link";
    ...
    panelOverlay.className = "adv-panel-overlay";
    ...
    shortcutHelp.id = "adv-shortcut-help";
    shortcutHelp.className = "adv-sr-only";
    shortcutHelp.textContent = i18n.t("shell.shortcutHelp");
    layout.appendChild(shortcutHelp);
    layout.setAttribute("aria-describedby", "adv-shortcut-help");
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
        ...
        if ((e.ctrlKey || e.metaKey) && e.key === "a" && !e.shiftKey) {
            ...
            if (tag !== "INPUT" && tag !== "TEXTAREA" && tag !== "SELECT") {
                e.preventDefault();
                const textLayers = viewportSlot.querySelectorAll<HTMLElement>(".adv-spread__text-layer");
    ...
    function applyThemeClass(isDark: boolean): void {
        layout.classList.toggle("adv-viewer-dark", isDark);
    }
    ...
    // Subscribe to panel state to toggle adv-panel-open class,
    ...
    const unsubPanelClass = store.subscribeRender((prev, next) => {
        if ((prev.activePanel === null) !== (next.activePanel === null)) {
            layout.classList.toggle("adv-panel-open", next.activePanel !== null);
        }
        ...
        if (prev.textSelectionDisabled !== next.textSelectionDisabled) {
            layout.classList.toggle("adv-viewer--no-text-select", next.textSelectionDisabled);
        }
    ...
    if (mergedInitialState.textSelectionDisabled) {
        layout.classList.add("adv-viewer--no-text-select");
    }
