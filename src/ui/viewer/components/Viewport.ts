...
export function createViewport(showAttribution = true, customPageOverlay?: CustomPageOverlayRenderer) {
    ...
    contentArea.className = "adv-viewport__content";
    el.appendChild(contentArea);

    const scrollArea = document.createElement("div");
    scrollArea.className = "adv-viewport__scroll";
    contentArea.appendChild(scrollArea);

    const container = document.createElement("div");
    container.className = "adv-viewport__container";
    ...
    // a runtime visibility tripwire fires `adv:branding-suppressed` if anything
    ...
    function mount(
        ...
        // theme MutationObserver can find `.adv-viewer-root`.
        branding?.start();
        workerClient = wc;
        storeRef = store;
        i18nRef = i18n ?? null;
        ...
        const handleAnnotationClick = (e: MouseEvent) => {
            ...
            const textEl = target.closest(".adv-annotation--text") as HTMLElement | null;
            if (textEl) {
                e.stopPropagation();
            ...
            const linkEl = target.closest(".adv-annotation--link");
            if (linkEl) {
        ...
        const viewerRoot = el.closest(".adv-viewer-root") as HTMLElement;
        if (viewerRoot) {
    ...
    function applyState(slice: ViewportSlice): void {
        ...
        el.classList.toggle("adv-viewport--seamless", slice.spacingMode === "none" || slice.viewMode === "continuous");
        el.classList.toggle("adv-viewport--continuous", slice.viewMode === "continuous");
            ...
            for (let i = visibleRange.start; i <= visibleRange.end; i++) {
                ...
                if (isContinuousView && contLayout?.isGridLayout) {
                    ...
                    const isLastRow = rowLayout
                        ...
                        : false;
                    spreadEl.classList.toggle("adv-spread--last-col", isLastCol);
                    spreadEl.classList.toggle("adv-spread--last-row", isLastRow);
                    ...
                    spreadEl.classList.add("adv-spread--last-col");
                    spreadEl.classList.toggle("adv-spread--last-row", i === state.layouts.length - 1);
                } else {
                    // Paged mode
                    spreadEl.style.left = "0px";
                    spreadEl.style.right = "0px";
                    spreadEl.style.width = "";
                    spreadEl.style.justifyContent = "";
                    spreadEl.classList.remove("adv-spread--last-col", "adv-spread--last-row");
                }
    ...
    function captureSnapshot(spreadComp: SpreadComponent): HTMLElement | null {
        ...
        const mainCanvas =
            el.querySelector<HTMLCanvasElement>(".adv-spread__canvas") ??
    ...
    function showSingleSpread(slice: ViewportSlice, _metrics: ViewportMetrics, state: LayoutState): void {
        ...
        if (snapshot && wantTransition) {
            ...
            const slot = spreadEl.querySelector<HTMLElement>(".adv-spread__slot");
            if (slot) {
                ...
                shadowEl.style.boxShadow = "var(--adv-shadow-page)";
