...
function createButton(className: string, label: string, iconSvg: string, keyshortcuts?: string): HTMLButtonElement {
    ...
    btn.className = `adv-toolbar__btn ${className}`;
...
export function createToolbar() {
    ...
    el.className = "adv-toolbar";
    el.setAttribute("role", "toolbar");
    el.setAttribute("aria-label", "Document toolbar");

    // Left section
    const leftSection = document.createElement("div");
    leftSection.className = "adv-toolbar__left";
    const menuBtn = createButton("adv-toolbar__btn--menu", "Menu", ICON_MENU);

    // Pointer split button
    const pointerSplitBtn = document.createElement("div");
    pointerSplitBtn.className = "adv-toolbar__split-btn";

    const pointerMainBtn = document.createElement("button");
    pointerMainBtn.className = "adv-toolbar__btn adv-toolbar__split-btn-main";
    pointerMainBtn.innerHTML = ICON_TOOL_POINTER;

    const pointerDropBtn = document.createElement("button");
    pointerDropBtn.className = "adv-toolbar__btn adv-toolbar__split-btn-drop";
    ...
    pointerDropdown.className = "adv-toolbar__split-dropdown";
    ...
    const annotateBtn = createButton(
        "adv-toolbar__btn--tool adv-toolbar__btn--annotate",
        "Annotate",
        ICON_TOOL_ANNOTATE,
    );
    const markupBtn = createButton("adv-toolbar__btn--tool adv-toolbar__btn--markup", "Markup", ICON_TOOL_MARKUP);
    ...
    centerSection.className = "adv-toolbar__center";
    centerSection.style.display = "none";

    // -- Page navigation
    const navGroup = document.createElement("div");
    navGroup.className = "adv-toolbar__group";

    const prevBtn = document.createElement("button");
    prevBtn.className = "adv-toolbar__btn adv-toolbar__btn--nav";
    prevBtn.innerHTML = ICON_CHEVRON_LEFT;
    prevBtn.title = "Previous page";
    prevBtn.setAttribute("aria-label", "Previous page");

    const pageInfo = document.createElement("div");
    pageInfo.className = "adv-toolbar__page-info";

    const pageInput = document.createElement("input");
    pageInput.className = "adv-toolbar__page-input";
    pageInput.type = "text";
    pageInput.inputMode = "numeric";
    pageInput.setAttribute("aria-label", "Page number");

    const pageTotal = document.createElement("span");
    pageTotal.className = "adv-toolbar__page-total";

    pageInfo.append(pageInput, pageTotal);

    const nextBtn = document.createElement("button");
    nextBtn.className = "adv-toolbar__btn adv-toolbar__btn--nav";
    ...
    divider1.className = "adv-toolbar__divider";

    // -- Zoom controls
    const zoomGroup = document.createElement("div");
    zoomGroup.className = "adv-toolbar__group";

    const zoomOutBtn = document.createElement("button");
    zoomOutBtn.className = "adv-toolbar__btn adv-toolbar__btn--nav";
    ...
    zoomDropdownContainer.className = "adv-zoom-dropdown adv-zoom-dropdown--toolbar";

    const zoomToggle = document.createElement("div");
    zoomToggle.className = "adv-zoom-dropdown__toggle";

    const zoomInput = document.createElement("input");
    zoomInput.className = "adv-zoom-dropdown__input";
    zoomInput.type = "text";
    ...
    zoomChevron.className = "adv-zoom-dropdown__chevron";
    ...
    zoomDropdown.className = "adv-zoom-dropdown__menu";
    ...
    zoomInBtn.className = "adv-toolbar__btn adv-toolbar__btn--nav";
    ...
    divider2.className = "adv-toolbar__divider";
    ...
    rightSection.className = "adv-toolbar__right";
    const searchBtn = createButton("adv-toolbar__btn--search", "Search", ICON_SEARCH, "Control+F Meta+F");
    const commentsBtn = createButton("adv-toolbar__btn--comments", "Comments", ICON_COMMENTS);
    const printBtn = createButton("adv-toolbar__btn--print", "Print", ICON_PRINT);
    const downloadBtn = createButton("adv-toolbar__btn--download", "Download", ICON_DOWNLOAD);
    const themeBtn = createButton("adv-toolbar__btn--theme", "Toggle theme", ICON_THEME_DARK);
    const fullscreenBtn = createButton("adv-toolbar__btn--fullscreen", "Fullscreen", ICON_FULLSCREEN);
    // Overflow menu (visible only on mobile)
    const overflowContainer = document.createElement("div");
    overflowContainer.className = "adv-overflow-menu";

    const overflowBtn = createButton("adv-toolbar__btn--more", "More", ICON_MORE);
    overflowBtn.setAttribute("aria-haspopup", "true");
    overflowBtn.setAttribute("aria-expanded", "false");

    const overflowDropdown = document.createElement("div");
    overflowDropdown.className = "adv-overflow-menu__dropdown";
    ...
    const openZoomDropdown = () => {
        if (!isZoomDropdownOpen) {
            ...
            zoomChevron.classList.add("adv-zoom-dropdown__chevron--active");
    ...
    const closeZoomDropdown = () => {
        if (isZoomDropdownOpen) {
            ...
            zoomDropdown.style.display = "none";
            zoomChevron.classList.remove("adv-zoom-dropdown__chevron--active");
            zoomChevron.setAttribute("aria-expanded", "false");
        }
    };
    ...
    function mount(container: HTMLElement, store: Store<ViewerState, Action>, i18n: I18n): void {
        ...
        function buildZoomDropdown(slice: ToolbarSlice, store: Store<ViewerState, Action>): void {
            ...
            stepsSection.className = "adv-zoom-dropdown__section";

            for (const step of slice.zoomSteps) {
                const item = document.createElement("button");
                item.className = "adv-zoom-dropdown__item";
                ...
                if (isActive) {
                    item.classList.add("adv-zoom-dropdown__item--active");
                }
            ...
            dividerEl.className = "adv-zoom-dropdown__divider";
            dividerEl.setAttribute("role", "separator");
            zoomDropdown.appendChild(dividerEl);

            // Zoom mode options
            const modeSection = document.createElement("div");
            modeSection.className = "adv-zoom-dropdown__section";
        ...
        rovingTabindex = setupRovingTabindex(el, ".adv-toolbar__btn, .adv-toolbar__btn--nav, input");
        ...
        function buildPointerDropdown(): void {
            ...
            for (const st of SIMPLE_TOOLS) {
                const item = document.createElement("button");
                item.className = "adv-toolbar__split-dropdown-item";
                const state = store.getState();
                const isActive = state.activeTool.kind === st.tool;
                if (isActive) item.classList.add("adv-toolbar__split-dropdown-item--active");
                item.innerHTML = `<span class="adv-toolbar__split-dropdown-icon">${st.icon}</span><span>${i18n.t(st.labelKey as keyof typeof i18n.t)}</span>`;
                item.addEventListener("click", (e) => {
                    e.stopPropagation();
        ...
        const onFullscreenClick = () => {
            const root = el.closest(".adv-viewer-root") as HTMLElement | null;
            if (!root) return;
        ...
        const onFullscreenChange = () => {
            const root = el.closest(".adv-viewer-root");
        ...
        const applyState = (slice: ToolbarSlice) => {
            ...
            // Rebuild overflow menu items
            {
                ...
                for (const item of items) {
                    ...
                    menuItem.className = "adv-overflow-menu__item";
                    menuItem.setAttribute("role", "menuitem");
                    menuItem.innerHTML = `<span class="adv-overflow-menu__item-icon">${item.icon}</span><span class="adv-overflow-menu__item-label">${item.label}</span>`;
            ...
            // Tool button active states
            {
                ...
                pointerSplitBtn.classList.toggle("adv-toolbar__split-btn--active", simpleActive);
                ...
                annotateBtn.classList.toggle("adv-toolbar__btn--tool-active", kind === "annotate");
                markupBtn.classList.toggle("adv-toolbar__btn--tool-active", kind === "markup");
            }
