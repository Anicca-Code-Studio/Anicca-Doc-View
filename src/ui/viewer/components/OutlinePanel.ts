...
export function createOutlinePanel() {
    ...
    el.className = "adv-outline-panel";
    ...
    function createOutlineItemElement(item: OutlineItem, path: string, depth: number): HTMLElement {
        ...
        container.className = "adv-outline-item";
        container.dataset.path = path;
        container.setAttribute("role", "treeitem");
        container.setAttribute("aria-label", item.title);

        const header = document.createElement("div");
        header.className = "adv-outline-item__header";
        ...
        if (item.children.length > 0) {
            ...
            toggle.className = "adv-outline-item__toggle";
            toggle.type = "button";
            ...
            toggle.classList.toggle("adv-outline-item__toggle--expanded", !isCollapsed);
            ...
            spacer.className = "adv-outline-item__spacer";
            header.appendChild(spacer);
        }

        // Title
        const title = document.createElement("span");
        title.className = "adv-outline-item__title";
        ...
        if (item.destination) {
            header.classList.add("adv-outline-item__header--clickable");
        ...
        if (item.children.length > 0) {
            ...
            childrenContainer.className = "adv-outline-item__children";
    ...
    function handleTreeKeyDown(e: KeyboardEvent, container: HTMLElement, path: string): boolean {
        const hasChildren = container.querySelector(":scope > .adv-outline-item__children") !== null;
        ...
        switch (e.key) {
            ...
            case "ArrowRight": {
                if (hasChildren && !isExpanded) {
                    ...
                    const firstChild = container.querySelector(
                        ":scope > .adv-outline-item__children > .adv-outline-item",
                    ) as HTMLElement | null;
            ...
            case "ArrowLeft": {
                if (hasChildren && isExpanded) {
                    ...
                    const parent = container.parentElement?.closest(".adv-outline-item") as HTMLElement | null;
                    if (parent) focusTreeItem(parent);
                }
                return true;
            }
            case "Home": {
                const first = el.querySelector(".adv-outline-item") as HTMLElement | null;
                if (first) focusTreeItem(first);
                return true;
            }
            case "End": {
                const allVisible = el.querySelectorAll<HTMLElement>(".adv-outline-item");
            ...
            case "Enter":
            case " ": {
                const header = container.querySelector(":scope > .adv-outline-item__header") as HTMLElement | null;
    ...
    function focusTreeItem(item: HTMLElement): void {
        const header = item.querySelector(":scope > .adv-outline-item__header") as HTMLElement | null;
        if (header) header.focus();
    }

    function getNextVisibleTreeItem(current: HTMLElement): HTMLElement | null {
        // If expanded, first child
        const children = current.querySelector(":scope > .adv-outline-item__children") as HTMLElement | null;
        if (children && children.style.display !== "none") {
            const firstChild = children.querySelector(":scope > .adv-outline-item") as HTMLElement | null;
        ...
        while (node) {
            ...
            if (next && next.classList.contains("adv-outline-item")) return next;
            // Go up to parent's next sibling
            node = node.parentElement?.closest(".adv-outline-item") as HTMLElement | null;
        }
        return null;
    }
    ...
    function getPrevVisibleTreeItem(current: HTMLElement): HTMLElement | null {
        ...
        if (prev && prev.classList.contains("adv-outline-item")) {
            // Go to deepest last visible descendant
            return getLastVisibleDescendant(prev);
        }
        // Go to parent
        const parent = current.parentElement?.closest(".adv-outline-item") as HTMLElement | null;
        return parent;
    }

    function getLastVisibleDescendant(item: HTMLElement): HTMLElement {
        const children = item.querySelector(":scope > .adv-outline-item__children") as HTMLElement | null;
        if (children && children.style.display !== "none") {
            const lastChild = children.querySelector(":scope > .adv-outline-item:last-child") as HTMLElement | null;
    ...
    function updateExpandState(container: HTMLElement, path: string): void {
        const toggle = container.querySelector(":scope > .adv-outline-item__header > .adv-outline-item__toggle");
        const children = container.querySelector(":scope > .adv-outline-item__children") as HTMLElement | null;
    ...
    function buildOutlineTree(outline: OutlineItem[]): void {
        ...
        const firstHeader = el.querySelector<HTMLElement>(".adv-outline-item__header");
    ...
    function showLoading(): void {
        ...
        loading.className = "adv-outline-panel__loading";
