...
export function createAnnotationPanel() {
    ...
    el.className = "adv-annotation-panel";

    let unsubRender: (() => void) | null = null;
    let storeRef: Store<ViewerState, Action> | null = null;
    let i18nRef: I18n | null = null;
    function renderPlaceholder(): void {
        el.innerHTML = `<div class="adv-panel-empty">${i18nRef!.t("annotations.noComments")}</div>`;
    }

    function renderLoading(): void {
        el.innerHTML = `
            <div class="adv-annotation-panel__loading">
    ...
    function renderCommentItem(container: HTMLElement, annotation: Annotation, pageIndex: number, depth: number): void {
        ...
        item.className =
            depth > 0
                ? `adv-comment-item adv-comment-reply adv-comment-depth-${Math.min(depth, 3)}`
                : "adv-comment-item";

        // Icon
        const icon = document.createElement("div");
        icon.className = "adv-comment-icon";
        icon.innerHTML = getAnnotationTypeIcon(annotation.type);
        item.appendChild(icon);

        // Body
        const body = document.createElement("div");
        body.className = "adv-comment-body";

        // Header with author and status
        const header = document.createElement("div");
        header.className = "adv-comment-header";

        const meta = annotation.metadata;
        const title = meta?.author || meta?.subject;
        if (title) {
            const titleEl = document.createElement("span");
            titleEl.className = "adv-comment-author";
        ...
        if (meta?.state) {
            ...
            badge.className = `adv-comment-status adv-comment-status--${meta.state.toLowerCase()}`;
        ...
        if (meta?.contents) {
            ...
            contents.className = "adv-comment-contents";
        ...
        if (annotation.replies && annotation.replies.length > 0) {
            ...
            toggleBtn.className = "adv-comment-toggle";
        ...
        if (annotation.replies && annotation.replies.length > 0) {
            const toggleBtn = body.querySelector(".adv-comment-toggle") as HTMLButtonElement;

            const repliesContainer = document.createElement("div");
            repliesContainer.className = "adv-comment-replies adv-comment-replies--collapsed";
            ...
            toggleBtn.addEventListener("click", (e) => {
                e.stopPropagation();
                const isCollapsed = repliesContainer.classList.contains("adv-comment-replies--collapsed");
                if (isCollapsed) {
                    repliesContainer.classList.remove("adv-comment-replies--collapsed");
                    toggleBtn.classList.add("adv-comment-toggle--expanded");
                    toggleBtn.title = i18nRef!.t("annotations.hideReplies");
                } else {
                    repliesContainer.classList.add("adv-comment-replies--collapsed");
                    toggleBtn.classList.remove("adv-comment-toggle--expanded");
                    toggleBtn.title = i18nRef!.t("annotations.showReplies");
                }
            });
        }
    ...
    function render(slice: AnnotationPanelSlice): void {
        ...
        list.className = "adv-comments-list";
        ...
        for (const pageIndex of sortedPages) {
            ...
            pageGroup.className = "adv-comments-page-group";

            // Page header
            const pageHeader = document.createElement("div");
            pageHeader.className = "adv-comments-page-header";
            pageHeader.innerHTML = `${ICON_CHEVRON_DOWN}<span>${i18nRef!.t("annotations.pageHeader", { page: pageIndex + 1 })}</span><span class="adv-comments-page-count">${annotations.length}</span>`;
            pageGroup.appendChild(pageHeader);

            // Page content
            const pageContent = document.createElement("div");
            pageContent.className = "adv-comments-page-content";
            ...
            pageHeader.addEventListener("click", () => {
                const isCollapsed = pageGroup.classList.contains("adv-comments-page-group--collapsed");
                if (isCollapsed) {
                    pageGroup.classList.remove("adv-comments-page-group--collapsed");
                } else {
                    pageGroup.classList.add("adv-comments-page-group--collapsed");
                }
            });
