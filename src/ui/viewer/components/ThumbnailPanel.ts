...
export function createThumbnailPanel() {
    ...
    el.className = "adv-thumbnail-panel";
    ...
    el.addEventListener("keydown", (e: KeyboardEvent) => {
        ...
        if (handled) {
            e.preventDefault();
            el.setAttribute("aria-activedescendant", `adv-thumb-${focusedPage}`);
    ...
    function createThumbnailItem(pageNumber: number, pageInfo: PageInfo, thumbnailWidth: number): ThumbnailItem {
        ...
        container.className = "adv-thumbnail-item";
        ...
        container.id = `adv-thumb-${pageNumber}`;

        const canvas = document.createElement("canvas");
        canvas.className = "adv-thumbnail-item__canvas";
        ...
        label.className = "adv-thumbnail-item__label";
    ...
    function updateCurrentPageHighlight(currentPage: number): void {
        for (const item of thumbnailItems) {
            ...
            item.container.classList.toggle("adv-thumbnail-item--active", isActive);
            item.container.setAttribute("aria-selected", String(isActive));
        }
        el.setAttribute("aria-activedescendant", `adv-thumb-${currentPage}`);
