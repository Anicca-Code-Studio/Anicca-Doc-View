...
export function createColorSelect(config: ColorSelectConfig): ColorSelectInstance {
    ...
    el.className = "adv-subtoolbar__dropdown-trigger";
    el.title = config.label;

    const swatch = document.createElement("span");
    swatch.className = "adv-subtoolbar__color-swatch";
    ...
    panel.className = "adv-color-panel";
    ...
    function updateSwatch(color: string | null): void {
        swatch.classList.remove(
            "adv-subtoolbar__color-swatch--stroke",
            "adv-subtoolbar__color-swatch--stroke-none",
            "adv-subtoolbar__color-swatch--fill",
            "adv-subtoolbar__color-swatch--fill-none",
        );
        swatch.style.removeProperty("--swatch-color");

        if (config.variant === "stroke") {
            swatch.classList.add("adv-subtoolbar__color-swatch--stroke");
            if (color) swatch.style.setProperty("--swatch-color", color);
            else swatch.classList.add("adv-subtoolbar__color-swatch--stroke-none");
        } else {
            swatch.classList.add("adv-subtoolbar__color-swatch--fill");
            if (color) swatch.style.setProperty("--swatch-color", color);
            else swatch.classList.add("adv-subtoolbar__color-swatch--fill-none");
        }
    ...
    function rebuildPanel(color: string | null): void {
        ...
        grid.className = "adv-color-panel__grid";
        grid.classList.add(
            config.variant === "stroke" ? "adv-color-panel__grid--stroke" : "adv-color-panel__grid--fill",
        );

        // None option
        const noBtn = document.createElement("button");
        noBtn.className = "adv-color-panel__swatch adv-color-panel__swatch--none";
        noBtn.classList.add(
            config.variant === "stroke"
                ? "adv-color-panel__swatch--none-stroke"
                : "adv-color-panel__swatch--none-fill",
        );
        if (color === null) noBtn.classList.add("adv-color-panel__swatch--active");
        ...
        for (const preset of COLOR_PRESETS) {
            ...
            sw.className = "adv-color-panel__swatch";
            if (color === preset.hex) sw.classList.add("adv-color-panel__swatch--active");
    ...
    function open(anchorRect: DOMRect, containerRect: DOMRect): void {
        ...
        panel.classList.add("adv-color-panel--open");
    }

    function close(): void {
        panel.classList.remove("adv-color-panel--open");
    }

    function isOpen(): boolean {
        return panel.classList.contains("adv-color-panel--open");
    }
