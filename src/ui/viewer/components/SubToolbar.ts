...
export function createSubToolbar() {
    ...
    el.className = "adv-subtoolbar";
    el.setAttribute("role", "toolbar");
    el.style.display = "none";

    const toolsSection = document.createElement("div");
    toolsSection.className = "adv-subtoolbar__tools";
    const dividerEl = document.createElement("div");
    dividerEl.className = "adv-subtoolbar__divider";
    const optionsSection = document.createElement("div");
    optionsSection.className = "adv-subtoolbar__options";

    // Undo/redo section (right end, after options)
    const undoDivider = document.createElement("div");
    undoDivider.className = "adv-subtoolbar__divider";
    undoDivider.style.display = "none";
    const undoSection = document.createElement("div");
    undoSection.className = "adv-subtoolbar__undo";
    ...
    function closeAllPanels(): void {
        ...
        lineStylePanel.classList.remove("adv-linestyle-panel--open");
        arrowHeadPanel.classList.remove("adv-arrowhead-panel--open");
    ...
    function openPanelAt(key: PanelKey, trigger: HTMLElement): void {
        ...
        if (key === "strokeColor") strokeColorSelect?.open(rect, cRect);
            ...
            lineStylePanel.classList.add("adv-linestyle-panel--open");
        } else if (key === "arrowHead") {
            arrowHeadPanel.style.left = left;
            arrowHeadPanel.classList.add("adv-arrowhead-panel--open");
        }
    ...
    function mount(
        ...
        if (undoManager) {
            ...
            undoBtn.className = "adv-subtoolbar__btn";
            ...
            redoBtn.className = "adv-subtoolbar__btn";
        ...
        const applyState = (slice: SubToolbarSlice) => {
            ...
            if (currentToolSet !== toolSet) {
                ...
                for (const def of subTools) {
                    ...
                    btn.className = "adv-subtoolbar__btn";
                    ...
                    btn.addEventListener("click", () => {
                        ...
                        if (currentSub === def.id && (def.id === "polygon" || def.id === "polyline")) {
                            document.dispatchEvent(new Event("adv-finish-drawing"));
                        } else {
            ...
            for (const [id, btn] of currentToolBtns) {
                btn.classList.toggle("adv-subtoolbar__btn--active", id === slice.activeSubTool);
            }
    ...
    function buildControls(subTool: SubTool, opts: ToolOptions, store: Store<ViewerState, Action>, i18n: I18n): void {
        ...
        if (subTool === "select") {
            ...
            deleteBtn.className = "adv-subtoolbar__btn adv-subtoolbar__btn--delete";
        ...
        for (const optKey of supported) {
            if (optKey === "strokeColor") {
                ...
                lineStyleTrigger.className =
                    "adv-subtoolbar__dropdown-trigger adv-subtoolbar__dropdown-trigger--icon";
                lineStyleTrigger.title = i18n.t("tools.lineStyle");
                const arrow = document.createElement("span");
                arrow.className = "adv-subtoolbar__dropdown-arrow";
                arrow.textContent = "\u25BE";
    ...
    function rebuildLineStylePanel(current: LineStyle, store: Store<ViewerState, Action>, i18n: I18n): void {
        ...
        for (const def of LINE_STYLE_DEFS) {
            ...
            item.className = "adv-linestyle-panel__item";
            if (current === def.id) item.classList.add("adv-linestyle-panel__item--active");
            const icon = document.createElement("span");
            icon.className = "adv-linestyle-panel__icon";
            icon.innerHTML = def.icon;
            const label = document.createElement("span");
            label.className = "adv-linestyle-panel__label";
    ...
    function rebuildArrowHeadPanel(
        ...
        for (const [rowKey, rowLabel, currentVal] of [
            ...
            row.className = "adv-arrowhead-panel__row";
            const lbl = document.createElement("span");
            lbl.className = "adv-arrowhead-panel__row-label";
            lbl.textContent = rowLabel;
            row.appendChild(lbl);
            for (const def of ARROW_HEAD_DEFS) {
                const btn = document.createElement("button");
                btn.className = "adv-arrowhead-panel__btn";
                if (currentVal === def.id) btn.classList.add("adv-arrowhead-panel__btn--active");
                btn.innerHTML = def.icon;
                btn.title = i18n.t(def.labelKey as keyof typeof i18n.t);
